...
peak_seqs <- read.fasta("DHS-minus-AHR_peaks_seqs.fa")
cat("--- No. of  peaks found = ", length(peak_seqs), "\n")
...
write.csv(longSites, file = "DHS-minus-AHR_peaks_AHREs_19bp.csv", row.names = FALSE)
...
# Write to bed file
f <- file("DHS-minus-AHR_peaks_AHREs_19bp.bed", "w") 
line = 'track name="unbound AHREs"  description="AHREs not under AHR peaks"  useScore=1'
writeLines(line, con = f)
close(f)
write.table(longSites2, file="DHS-minus-AHR_peaks_AHREs_19bp.bed",
